...
ax2    = nexttile(tl_d);
ymax_f = max([d.freq(row_idx,:), d.meanFreq]) * 1.15;
if ymax_f <= 0, ymax_f = 1; end
fill(ax2, [2 4 4 2], [0 0 ymax_f ymax_f], ...
...
ylabel(ax2, 'Power (\DeltaF/F)^2 Hz^{-1}', 'FontSize', 7);
title(ax2, 'Pre-stim spectrum: this trial (red) vs mean (grey). 2-4 Hz band shaded.', 'FontSize', 7);

% --- Panel 3: metadata text ---
ax3 = nexttile(tl_d);
axis(ax3, 'off');
dev_rank = sum(d.dev <= d.dev(row_idx));
band24   = d.fbCtrs >= 2 & d.fbCtrs <= 4;
pwr24    = mean(d.freq(row_idx, band24), 'omitnan');
pwr24_mn = mean(d.meanFreq(band24), 'omitnan');
info_str = sprintf([ ...
    ...
    '2-4 Hz power this trial             : %.5f   (mean: %.5f)'], ...
    row_idx, d.nT, ...
    d.pvar(row_idx), ...
