...
- [x] **[T2] Variance-tautology — ⚠ CONFIRMED BITING (2026-07-01)** — tested devL1(stim) vs devP(no-stim pre window): corr(devP,PreVar)=+0.395 ≥ raw +0.359; excess corr(devL1−devP,state)≈0; binned curves superimposed. The variance/δ "local state-dependence" is state-dependent PREDICTABILITY (contra→ipsi degrades in synced states, stim or not), NOT a state-dependent local stim response. Partial +0.213 survives only via regression dilution. **→ REFRAME the claim** (see below); motion result unaffected. RESEARCH 2026-07-01.
  - [ ] REFRAME manuscript: "local photoinhibition response is state-dependent" → "inter-hemispheric predictability is state-dependent" (synced state→less predictable→higher CL MSE = cleaner controller bridge). Re-check FINDINGS.md "state-robust to motion" headline for the variance/δ caveat.
  - [ ] Confirm the predictability effect with an INDEPENDENT state (pupil) to break the PreVar/devP tautology.
  - [ ] Productionize the stim-vs-no-stim excess test as a `[CP-PREDQ]` control section (helper `cp_predq.m`).
