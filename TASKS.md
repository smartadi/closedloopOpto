...
- [ ] **[T2] Variance-tautology** — PreVar partial 0.70 (Actual/Global); Local 0.213 may be residual variance-autocorrelation not state modulation. Control: match/regress `dev_pre` harder, OR use an INDEPENDENT state estimate (pupil) instead of the imaging signal's own variance. **Tractable now.**
