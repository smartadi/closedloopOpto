...
- [~] 2026-07-13 — **Dynamical NETWORK model** on the 52-site tensor: `bilateral/grid/network_id.py`. Verdict: latent states required (1st-order observed-state caps at free-run R²~0.3). **Stabilized delay-DMD = the predictive model (R² 0.64, stable)**; directed 1st-order Laplacian = interpretable connectivity (sparse/local, not predictive); simulation-error structured 2nd-order Laplacian = stable but rigid (R² 0.12). Parsimony↔accuracy tradeoff. **Next: driven form (input B, opsin sign) for prediction of NEW stim patterns.** RESEARCH 2026-07-13.
