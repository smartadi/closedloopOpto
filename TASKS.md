...
- [ ] **[NEXT] Run Stage 1** on m4 (needs `load_sessions.m`; ROI GUI opens once). Success criteria: held-out R² in the impulse pipeline's range (~0.85–0.95), and `corr(rebuilt, data.dFk)` high enough to confirm the pixel convention. Then sweep the other 11 usable sessions.
