...
> Scope fixed by the user 2026-08-02: **four impulse sessions** = AL_0041 e1, AL_0041 e2, AL_0033 2025-01-29 e1, and **AL_0048 2026-07-15 e1 with TWO sites** (left excitatory / right inhibitory) = 5 site-datasets. AL_0048 is now registered in MATLAB via `impulse-analysis/load_bilateral_impulse.m` (RESEARCH 2026-08-02).
...
- [ ] **[USER DECISION] `BLI.SITE_MODE` for the inhibitory side** — 'calibrated' (illuminated spot, weak/non-monotone dose) vs 'response' (anterior focus 2.6 mm away, clean dose). See the ⚠ RESEARCH entry 2026-08-02; check expression/histology before choosing.
- [ ] Extend the STATEDEP/STATEDEP-VAR sections (`ols_tf_pipeline.m` §17c/§17c2) to run headless per session and combine — the machinery is single-session today, and `local_stimblind_session` (§18) returns only per-AMP trial averages, not the per-trial Local dips the state statistic needs. Needs a per-trial Global/Local extraction inside the session function.
