...
### ⭐ PRIMARY stim-blind script: `ols_tf_pipeline.m` (since 2026-07-13)
Linear stream: SETUP → **Stage 1** TF affected-detection (§10T order-lock → §10T2 per-pixel fixed-order
fit → §10T3 interactive SELECTOR) → **Stage 2** stim-blind model (§17d `[STIMBLIND-SELECT]`) →
**Stage 3** residual state-dependence (§17c/§17c2) → **Stage 4** cross-session batch (§18 `[ALLSESS]`)
→ APPENDIX explorers. TF-map disk cache keyed by session+params (`tf_affected_<mn>_<date>_e<en>.mat`).
- **§17d SELECT is the SOLE stim-blind model** (2026-07-18: §17b NATIVE + §17bV retired, §18 repointed,
  §17c state-dep repointed to `bUseS`, `native_project` deleted). Theory: blindness is **earned, not
  imposed** — pixels are excluded by an independent criterion (TF match to the canonical ipsi impulse
  TF), then a sparse L1 fit with a spatial penalty growing toward the ipsi site keeps predictors FEW and
  FAR. **No dip constraint**, so the Global dip is an HONEST LEAK and capture = 100 − leak is falsifiable.
  NATIVE's exact KKT dip-cancellation guaranteed ~100% capture and was therefore uninformative.
  ⇒ Any pre-2026-07-18 NATIVE/GREEDY number is NOT comparable.
- **Gate gotcha:** §10T3 BLOCKS on `uiwait` until CONFIRM. `RUN_ALLSESS=true` alone does NOT reach §18 —
  press CONFIRM, or pass `OLS_OVERRIDE.affect_mode='matched'` to run headless. RESEARCH 2026-08-05.
- **§18 does NOT use the TF detector** — its engine `local_stimblind_session` keys off the energy-based
  `bledA` map. Reconcile before any cross-session number reaches the paper.
