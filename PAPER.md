...
| 5E | Fig5 | paper/images/figure5/sine_5E_rmse_time_AL_0048_2026-07-21_1.pdf | 5.2 × 4 | vector | ±SEM | **RMSE** over time, 4 modes; physical row 1. (s3 early→late: OL −0.40, OL+p +0.09, CL +0.06, CL+p −0.02 — do NOT repeat the old "CL+prev only mode that falls" claim; it's false, OL falls most here). **2026-08-12: y bar 2→1 and relabelled `1% ΔF/F`; rotated text cut to `RMSE`** |
| 5F | Fig5 | paper/images/figure5/sine_5F_variance_AL_0048_2026-07-21_1.pdf | 5.2 × 4 | vector | — | across-trial variance over time, 4 modes; physical row 1. **2026-08-12: y scale bar REMOVED entirely** (units are (%ΔF/F)²; a bare "10" was uninformative) — only the `1 s` bar + the rotated quantity name remain |
| ~~5G~~ | — | ~~sine_5G_rmse_violin~~ | — | — | — | **DROPPED 2026-07-29** — replaced by combined 5I (across-session RMSE). **REBUILT 2026-08-12** (log y axis, no p95 clip, all trials on-axis, median+IQR, p<0.05 brackets) + a `sine_5G_rmse_ecdf_<sfx>.pdf` alternative. Still not a registered panel; **note it only shows an effect on s2** (OL-vs-CL p=7.7e-4) — on s3 every pairwise p ≥ 0.30 and it renders bracket-free |
...
| 5J | Fig5 | paper/images/figure5/sine_combined_variance.pdf | 5.5 × 4.5 | vector | — | **across-session** total variance, 4 modes × 3 sessions; sine_ff_across_sessions.m. CL<OL in all 3. **2026-08-12: short-corner axes**, y bar `5 (% ΔF/F)²` |
| 5K | Fig5 | paper/images/figure5/sine_combined_phase.pdf | 5.5 × 4.5 | vector | — | **across-session** 1 Hz phase lag, 4 modes × 3 sessions; sine_ff_across_sessions.m. Preview→~0° in all 3. **2026-08-12: short-corner axes**, y bar `20°`, zero line kept |

> **⚠ ARTWORK MISMATCH (found 2026-08-12).** The assembled `paper/images/figure5/Figure5.pdf` is built from the **s2 (`2026-07-14/1`)** exports, not the s3 set registered below — panel C reads n = 51/46/46/57 and panel H reads 60/5/36/−9 with a 51° preview line, both s2 signatures. It also still places the two DROPPED panels (G and H), so the RMSE claim appears twice (G + I) and the phase claim twice (H + K). **Decide one:** (a) re-link the placed panels to `*_AL_0048_2026-07-21_1.pdf` and delete G/H, or (b) revert the primary to s2 here — defensible, since s2's within-session OL-vs-CL is p=7.7e-4 (s3's is n.s. p=0.36), which is the only way 5G can carry a within-session stat. Both sessions' panels are regenerated with the 2026-08-12 axis treatment.
>
