...
| 2C | Fig2 | paper/images/figure2/tf_cv_shape_across_sessions.pdf | 4.2 × 3.6 | vector | — | **NEW 2026-08-24 — CROSS-VALIDATED shape overlay is now 2C** (user: "this figure will become 2C"). Trial-level split-half CV: measured **held-out** h(t) solid vs the prediction of a model that never saw those trials, dashed, peak-normalised on the measured trace, one pair per session on the session gradient. Legend carries each session's **held-out R²** (s1 0.50 / s2 0.86 / s3 0.95 / s4 0.84); solid=measured / dashed=fit goes in the caption (no footer). `impulse-analysis/imp_tf_cv.m` → `utils/imp_tf_cv_session.m`. ⚠ dashed = MEAN prediction over 200 splits, so s1's instability (IQR 0.05–0.76) is not visible on the overlay — state it. Order fixed at the full-data selection across folds (CV validates dynamics, not order-selection). |
