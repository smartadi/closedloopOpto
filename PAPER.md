...
| 2A | Fig2 | paper/images/figure2/imp_response.pdf | 5 × 4 | vector | — | **SWAPPED to 2A 2026-08-24** (user: "imp_response is new fig 2A"). dose_response.m — **REAL Y AXIS + mouse labels** (`DR_YAXIS=true`, `DR_LABEL='mouse'` script defaults). y = "Inhibition energy (% ΔF/F, 0-200 ms)" with ticks (replaces the 0.01 corner scale bar); direct per-animal labels Mouse 1a/1b/2/3 (AL_0041 = 1a+1b). x keeps the `0.25 mW` scale bar. |
| 2B | Fig2 | paper/images/figure2/imp_single_AL_0033_2025-01-29_en1.pdf | 5 × 4 | vector | ±1 SD | **SWAPPED to 2B 2026-08-24** (user: "imp_single is the new fig 2B"). trace_overlay.m — ±1 SD ribbon (subset lowest+highest amp; faint fill α=0.08 + envelope outlines) |
| 2D (assembled) | Fig2 | paper/images/figure2/tf_cv_heldout_r2.pdf | 4.2 × 3.6 | vector | IQR | **CHANGED 2026-09-13** (user: "only use held out r2 image thats the best, keep others for supplementary"). Cross-validated generalization as a dot-plot: per-session **held-out R²** median (o) + IQR whisker on the session colour gradient, dashed pooled-median guide = **0.857**. Trial-level split-half CV (100 splits ×2 dir); s1 0.50 [0.05 0.76] / s2 0.86 / s3 0.95 / s4 0.84. No traces or text overlaid on data. `impulse-analysis/imp_tf_cv.m`. Caption: order fixed at full-data selection across folds (CV validates dynamics, not order-selection). ⚠ registry letter historically "2C" but this panel assembles at **D** (see figures_final README) — reconcile the letter in a dedicated pass. |
| 2-supp CV shape | supp | paper/images/figure2/tf_cv_shape_across_sessions.pdf (+ tf_cv_2D_sidebar / _endlabels) | 4.2 × 3.6 | vector | — | **SUPPLEMENTARY as of 2026-09-13** (demoted from main). CV shape overlay: measured held-out h(t) solid vs prediction dashed, peak-normalised, one pair/session; legend carries held-out R². Sidebar variant adds the R² side panel; endlabels variant direct-labels R² at trace ends. ⚠ dashed = MEAN over 200 splits, so s1's instability (IQR 0.05–0.76) is not visible on the overlay — state it. `impulse-analysis/imp_tf_cv.m` (CV_REVAMP block). |
| ~~2C-old~~ | ~~Fig2~~ | paper/images/figure2/tf_data_vs_model_AL_0033_2025-01-29_en1.pdf | 6 × 4 | vector | ±std | **SUPERSEDED 2026-08-24** by the CV 2C above. Was the single-session in-sample TF fit. |
| ~~2C-i~~ | ~~Fig2~~ | paper/images/figure2/tf_shape_across_sessions.pdf | 6 × 4 | vector | — | **SUPERSEDED 2026-08-24 by CV 2C** — this is the IN-SAMPLE shape overlay (still written by `imp_tf_run.m`; keep as the explore/diagnostic version, do NOT assemble). Peak-normalised h(t), measured solid / fit dashed. `utils/imp_tf_paper_fig.m` |
| TF-A | Fig2 | paper/images/figure2/tf_tau_forest.pdf | 5 × 4 | vector | 95% CI | **REBUILT 2026-08-17.** τ_slow **and τ_fast** per session (filled circle / open square, session colour, **log x**) + trial-bootstrap CI, over the cross-session mean ± SD band. Ticks s1…sN. ⚠ **CI is right-censored:** 42/50/2/50% of bootstrap resamples returned a τ longer than the 0.5 s fit window and were dropped as non-identifiable, so the upper bounds (471/502/478/498 ms) sit at the 510 ms cut. Quote the point estimate with the rejection fraction, never the upper CI. Between/within ratio **1.55** ("mild inter-session variability") — the old "0.00 → one shared time constant" was a bootstrap blow-up artefact |
| TF-P | Fig2? | paper/images/figure2/tf_pole_spectrum.pdf | 5 × 4 | vector | — | **UNREGISTERED, built 2026-08-17** by `utils/imp_tf_poles_fig.m` (untracked). Every stable time constant per session, log axis, complex pairs marked, order annotated. Overlaps TF-A substantially — **pick one before assembly**, do not ship both |
...
| TF-D | Fig2 | paper/images/figure2/tf_model_swap.pdf | 4.5 × 4 | vector | — | **cross-session model-swap R²** matrix: session i's fitted model against session j's measured h(t), **free gain** (gain is re-tuned per session by construction, so this is a claim about *dynamics*). ⚠ **CLAIM REVERSED 2026-08-13 on the 4-session set.** Was "Diagonal ≈ off-diagonal ⇒ designs transfer"; measured is **self 0.991 vs cross 0.714 (drop 0.277)**, with s2→s1 as low as 0.09, and the order is not shared either (np = [3 4 4 4]). The supported sentence is "a low-order LTI model fits **each** session; per-session identification is required" — **not** "the same model transfers". Methods must change with the panel. Ticks are s1…sN as of 2026-08-12 |
...
| 2J | Fig2 | paper/images/figure2/imp_state_var_motion.pdf | 6 × 4 | vector | 95% CI band | **RAW-UNITS REBUILD 2026-08-24** (NeuroAI setup, user). Now **Prediction error (% ΔF/F)** by **motion quartile** (falls Q1→Q4), 1767 trials / 4 sessions, `STVF_UNITS='raw'`, single red series, **no stim-free control** (raw has no control counterpart). Same trial pool as the 2026-08-12 build → same underlying stats: Q4/Q1 = 0.71 [0.60–0.84], stratified ρ = −0.120, 4/4 sessions. Shares its y-axis with 2K. `impulse-analysis/imp_state_trialvar_fig.m` with `STVF_PAPER=true`, `STV_MOTEXCL=false`. x is a **within-session percentile**, not a magnitude — motion is rest-plus-bouts and every magnitude axis crushes all four bins into the left tenth. The magnitude statement is the threshold split (z>1.5, ratio 0.43); pair them in the caption. ⚠ The "response tightens MORE than the ongoing signal" comparison (residual ratio 0.25 vs sham 0.41) is no longer on the panel — state it in the caption. |
| 2K | Fig2 | paper/images/figure2/imp_state_var_reldelta.pdf | 6 × 4 | vector | 95% CI band | **RAW-UNITS REBUILD 2026-08-24** (NeuroAI setup, user). Same raw **Prediction error (% ΔF/F)** axis as 2J (shared), by **relative-δ quartile** (rises Q1→Q4). Single red series, **no stim-free control on the panel**. Same trial pool as the 2026-08-12 build → same stats: Q4/Q1 = 1.19 [1.04–1.37], stratified ρ = +0.089, 3/4 sessions (s1 dissents). ⚠ **Still a NEGATIVE result — the caveat now lives in the caption, not the panel:** δ does NOT beat its stim-free control (1.19 stim vs 1.35 sham) and did not replicate on Ye/Zhiwen `AB_0004` (ρ=−0.25, opposite sign, RESEARCH 2026-07-02). Include only as the *negative* half of the pair — motion changes response variability, δ does not beyond what it does to ongoing signal. `impulse-analysis/imp_state_trialvar_fig.m`, `STV_MOTEXCL=false`. |
...
| 3G | Fig3 | paper/images/figure3/all_average_sessions.pdf | 3 × 4 | vector | ±SEM | step_response.m fig_H — per-session faint + bold mean. **METRIC = RMSE since 2026-08-24** (was MAE; user: "everything else has RMSE"). y = "RMSE dF/F", `sqrt(mean(e² over trials))`, ylim now data-driven (RMSE runs above the old MAE range). ⚠ now shares across-trial variance with 3F. |
...
>
> ⭐ **SUPERSEDED 2026-09-14 (user SELECTED build).** Row-3 metric is now **RR = ‖A−ref‖²/‖G‖²**
> (response energy over disturbance energy, **G referenced to ZERO** — user: disturbance = global minus
> zero, response = response minus reference), settled 1–3 s, **ridge** predictor. On the settled window
> **RR == ρ²**, so it is the same family as the retired ρ but reported as an energy ratio. The ER (G→ref)
> and SR (fluctuation) generations above are NOT what ships. **Permanent script:
> `controller-analysis/f4_disturb_rejection.m`** (promotes the 2026-09-11 scratchpad panels; metric folded
> into `imp_reject_core.m` as `R.rr_*`). Exemplar = **AL_0033_0415_e2** (most stim-blind). Panels →
