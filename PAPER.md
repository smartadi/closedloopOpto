...
> **✅ Fig 5 session REVERTED to s2 (2026-08-12, user decision).** The assembled `Figure5.pdf` had been built from s2 all along (panel C n = 51/46/46/57; panel H 60/5/36/−9 with a 51° preview line) while the registry said s3 — resolved in s2's favour rather than by re-linking, because **s2's within-session OL-vs-CL is p=7.7e-4** where s3's is n.s. (p=0.36), and that is the only way 5G carries a within-session statistic. `SESSION_TAG='s2'` is now the script default; all 5B–5G paths above are the `AL_0048_2026-07-14_1` set. **5G is un-dropped** and back in the panel table. **5H stays dropped** — its phase claim duplicates 5K regardless of session, so the assembled artwork should lose panel H (it currently still places it). s3 remains one of the three points in 5I/5J/5K and keeps its role in the presentation/pupil video.
>
