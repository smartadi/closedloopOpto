...
| 2B | Fig2 | paper/images/figure2/imp_response.pdf | 5 × 4 | vector | — | dose_response.m — **REAL Y AXIS + mouse labels since 2026-08-24** (`DR_YAXIS=true`, `DR_LABEL='mouse'` now the script defaults). y = "Inhibition energy (% ΔF/F, 0-200 ms)" with ticks (replaces the 0.01 corner scale bar); direct per-animal labels Mouse 1a/1b/2/3 (AL_0041 = 1a+1b). x keeps the `0.25 mW` scale bar. ⚠ corner-axes convention now differs from sibling 2A etc. |
