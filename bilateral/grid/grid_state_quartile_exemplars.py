...
for b in range(NBIN):
    ax = axes[b]; T = Qtr[b]
    m = np.nanmean(T, 0); sd = np.nanstd(T, 0)
    ax.fill_between(window, m - sd, m + sd, color=cols[b], alpha=0.22)
    ax.plot(window, m, color=cols[b], lw=1.9)
    ax.plot(window, expected, color="0.25", lw=1.2, ls="--")
    ax.axhline(0, c="0.8", lw=0.5); ax.axvline(0, c="0.8", lw=0.5)
    ax.set_xlim(-0.1, 0.5); ax.set_xticks([0, 0.2, 0.4])
...
axes[NBIN - 1].plot([], [], "0.25", ls="--", lw=1.2, label="expected (drive-only)")
axes[NBIN - 1].legend(fontsize=7, frameon=False, loc="upper right",
                      handlelength=1.2, borderaxespad=0.2)
fig.suptitle("Actual response (mean $\\pm$SD) vs expected, by pre-stim state quartile", fontsize=9)
fig.tight_layout()
