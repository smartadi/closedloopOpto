...
%   mn          td             en  trials   pixel_L      pixel_R
  ...
  'AL_0048', '2026-07-21',    1,  258,   [NaN NaN],   [407 367];  % sine FF-analysis, 4 modes, right side. 258 trials = 7 warm-up + 200 sine (two 100-trial iterations, ONE continuous kk clock, already unified in input_params) + 51 stationary-reference (ff_cond=-1 -> auto-excluded by the mode filter; IGNORED for now per 2026-07-22). pixel_R SET EXPLICITLY: params.pixel auto-resolved to [153 367] (LEFT/contra) because params.galvo_positions_mm(x) has a FLIPPED SIGN (-2.197) vs the true commanded galvo +2.18 (input_params col 15). Correct right-side pixel = bregma_x + |galvo_x|*mult = 280 + 2.197*57.80 = 407; y = 245 - (-2.111)*57.80 = 367. Verified: OL p2p 4.57% at [407 367] vs 1.58% at the buggy pixel (s2 = 5.81%).
};
