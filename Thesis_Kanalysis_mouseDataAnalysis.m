...
%% spontaneous data properties

sp = [data.pwcDfk(:,1:35*5);data.pncDfk(:,1:35*5)];
...
for k = 10:10:150
    r = randi(150, 1 , k);
    aa = sp(r,:);
    d2 = [d2,sum(vecnorm(aa - mean(aa),2,2))/(k*175)];
...
plot(td1,bb(1,:),'r','Linewidth',3);hold on
plot(td1,bb(end,:),'k','Linewidth',3);hold on
...
k=10:10:150;
figure;
boxplot(DD', 'Widths', 0.3);

xlabel('Row Index');
ylabel('Value');
skip = 2;
rowNumbers = 1:size(X, 1);
set(gca, 'XTick', 1:skip:size(DD, 1), ...
         'XTickLabel', k(1:skip:end));
xlabel('Trial batch size')
ylabel('dF/F variance')
title('Variance over batch of trials');
